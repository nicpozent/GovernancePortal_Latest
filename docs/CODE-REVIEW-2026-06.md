...
| Structure | ✅ Done | The shared effective-membership query is now a single DB view (`effective_group_membership`, migration 018). `routes.js` (~1.2k lines) was split into per-domain modules under `src/routes/` with shared helpers in `src/authz.js`/`src/uploads.js`; verified behaviour-preserving by an identical 64-route inventory + the unit/integration suite (`docs/TESTING.md`). |
...
- **`routes.js` monolith — resolved.** The former ~1,200-line file was split into per-domain modules under `src/routes/` (`policies.js`, `quizzes.js`, `groups.js`, …), each mounted on a shared router by `routes/index.js`; the shared helpers (`audit`, `canRead`, `canManage`, `teamOids`, roles) now live in `src/authz.js` and upload handling in `src/uploads.js`. The split was verified behaviour-preserving (identical 64-route inventory + green test suite).

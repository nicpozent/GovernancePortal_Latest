...
// Normalize a { steps:[{ approverOids, rule, required }] } body into a clean list.
...
const quorumOf = (s) => (s.rule === 'quorum' ? Math.min(Math.max(s.required || 1, 1), s.approverOids.length) : null);

// Persist a template's steps (delete-then-insert; cascades approvers).
...
// Load a template with its ordered steps + approvers.
async function loadWorkflow(id) {
  ...
  w.steps = stepRows.map((s) => ({ position: s.position, rule: s.rule, required: s.required, approvers: byPos.get(s.position) || [] }));
  return w;
}
...
module.exports = (r) => {
  ...
  r.post('/policies/:id/apply-workflow', async (req, res) => {
    ...
    res.json({ ok: true, steps: w.steps.length, approvers: w.steps.reduce((n, s) => n + s.approvers.length, 0) });
  });
};

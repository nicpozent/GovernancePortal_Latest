...
const TYPE_META = { Policy:'#213a9e', Process:'#0078c0', Procedure:'#6d4bd1', Standard:'#1f7a5c', Guideline:'#b7791f' };
export const pctColor = (p) => (p >= 80 ? '#1f7a5c' : (p >= 50 ? '#b7791f' : '#c0143c'));
export const typePill = (t) => { const c = TYPE_META[t] || '#6b7280'; return css(`display:inline-block;padding:4px 9px;border-radius:999px;font:600 10.5px/1.3 "IBM Plex Mono",monospace;letter-spacing:.06em;text-transform:uppercase;color:${c};background:${c}15`); };
export const statusPill = (st) => { const m = { signed:['#1f7a5c','#e6f3ec'], pending:['#9a6712','#fbf2df'], outdated:['#c0143c','#fbe7ec'] }[st] || ['#6b7280','#eef1f5']; return { display:'inline-flex', alignItems:'center', gap:'6px', padding:'5px 11px', borderRadius:'999px', fontFamily:'"IBM Plex Sans",sans-serif', fontSize:'12px', fontWeight:600, lineHeight:1, color:m[0], background:m[1] }; };
export const sourceStyle = (s) => { const c = s==='Entra ID' ? '#0078c0' : (s==='Active Directory' ? '#1f7a5c' : '#6b7280'); return css(`display:inline-flex;align-items:center;gap:6px;padding:3px 9px;border-radius:6px;font:500 11.5px/1.3 "IBM Plex Mono",monospace;color:${c};background:${c}12`); };
export const seg = (on) => ({ flex:'1', padding:'8px 16px', borderRadius:'8px', border:'none', cursor:'pointer', fontFamily:'"IBM Plex Sans",sans-serif', fontSize:'13px', fontWeight:600, lineHeight:1, background:on?'#fff':'transparent', color:on?'#213a9e':'#6b7280', boxShadow:on?'0 1px 3px rgba(20,30,80,.14)':'none' });
export const tabStyle = (on) => ({ padding:'8px 15px', borderRadius:'9px', border:'1px solid '+(on?'#213a9e':'#e6e8ee'), background:on?'#213a9e':'#fff', color:on?'#fff':'#41485a', cursor:'pointer', fontFamily:'"IBM Plex Sans",sans-serif', fontSize:'13px', fontWeight:600, lineHeight:1 });
export const chipStyle = (on) => ({ padding:'6px 13px', borderRadius:'999px', border:'1px solid '+(on?'#213a9e':'#e6e8ee'), background:on?'#eef1fb':'#fff', color:on?'#213a9e':'#6b7280', cursor:'pointer', fontFamily:'"IBM Plex Sans",sans-serif', fontSize:'12.5px', fontWeight:600, lineHeight:1 });

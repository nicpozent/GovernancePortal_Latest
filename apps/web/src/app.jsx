...
const API_BASE = CFG.API_BASE || '';                 // '' => same origin (nginx proxies /api)
const CONFIG_OK = !!(CFG.TENANT_ID && CFG.SPA_CLIENT_ID && CFG.API_CLIENT_ID);
...
const currentAccount = () => (msalApp ? msalApp.getActiveAccount() : null);
async function signIn() { await initAuth(); await msalApp.loginRedirect(loginRequest); }
async function signOut() { await initAuth(); await msalApp.logoutRedirect(); }
...
const IDLE_LOGOUT_MS = 15 * 60 * 1000;   // sign the user out after 15 min of inactivity
...
async function uploadRequest(method, path, fields, file) {
  ...
  if (!res.ok) { let msg = res.status + ''; try { const j = await res.json(); msg = (j.detail || j.error || msg); } catch (_) {} throw new Error(msg); }
  return res.status === 204 ? null : res.json();
}
const api = {
...
const pctColor = (p) => (p >= 80 ? '#1f7a5c' : (p >= 50 ? '#b7791f' : '#c0143c'));
const typePill = (t) => { const c = TYPE_META[t] || '#6b7280'; return css(`display:inline-block;padding:4px 9px;border-radius:999px;font:600 10.5px/1.3 "IBM Plex Mono",monospace;letter-spacing:.06em;text-transform:uppercase;color:${c};background:${c}15`); };
const statusPill = (st) => { const m = { signed:['#1f7a5c','#e6f3ec'], pending:['#9a6712','#fbf2df'], outdated:['#c0143c','#fbe7ec'] }[st] || ['#6b7280','#eef1f5']; return { display:'inline-flex', alignItems:'center', gap:'6px', padding:'5px 11px', borderRadius:'999px', fontFamily:'"IBM Plex Sans",sans-serif', fontSize:'12px', fontWeight:600, lineHeight:1, color:m[0], background:m[1] }; };
const sourceStyle = (s) => { const c = s==='Entra ID' ? '#0078c0' : (s==='Active Directory' ? '#1f7a5c' : '#6b7280'); return css(`display:inline-flex;align-items:center;gap:6px;padding:3px 9px;border-radius:6px;font:500 11.5px/1.3 "IBM Plex Mono",monospace;color:${c};background:${c}12`); };
const seg = (on) => ({ flex:'1', padding:'8px 16px', borderRadius:'8px', border:'none', cursor:'pointer', fontFamily:'"IBM Plex Sans",sans-serif', fontSize:'13px', fontWeight:600, lineHeight:1, background:on?'#fff':'transparent', color:on?'#213a9e':'#6b7280', boxShadow:on?'0 1px 3px rgba(20,30,80,.14)':'none' });
const tabStyle = (on) => ({ padding:'8px 15px', borderRadius:'9px', border:'1px solid '+(on?'#213a9e':'#e6e8ee'), background:on?'#213a9e':'#fff', color:on?'#fff':'#41485a', cursor:'pointer', fontFamily:'"IBM Plex Sans",sans-serif', fontSize:'13px', fontWeight:600, lineHeight:1 });
const chipStyle = (on) => ({ padding:'6px 13px', borderRadius:'999px', border:'1px solid '+(on?'#213a9e':'#e6e8ee'), background:on?'#eef1fb':'#fff', color:on?'#213a9e':'#6b7280', cursor:'pointer', fontFamily:'"IBM Plex Sans",sans-serif', fontSize:'12.5px', fontWeight:600, lineHeight:1 });
const MONTHS = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
const fmtDate = (v) => { if (!v) return '—'; const d = new Date(v); if (isNaN(d)) return '—'; return d.getDate()+' '+MONTHS[d.getMonth()]+' '+d.getFullYear(); };
const fmtDT = (d) => fmtDate(d.getTime())+', '+String(d.getHours()).padStart(2,'0')+':'+String(d.getMinutes()).padStart(2,'0');
...
/* small inline icons */
const Ico = ({ d, size=18, sw=2, children }) => (
...
const HELP_TOPICS = [
  ...
  { aud:'user', cat:'Privacy', title:'How long your records are kept', keywords:'retention how long kept delete erasure leavers 10 years gdpr right to be forgotten', body:[
    'Acknowledgement, audit and quiz records are kept for 10 years. This retention period exists because the records are legal compliance evidence — they may be needed long after a policy was signed, including after someone has left the company.',
    'Because of this, your records are retained even if you leave (this is a recognised exception to the GDPR right to erasure, where data must be kept to meet a legal obligation). When the 10-year retention period ends, the records are deleted.' ] },
  ...
  { aud:'admin', cat:'Data protection', title:'GDPR — privacy notice, retention & sub-processors', keywords:'gdpr privacy notice retention 10 years erasure leavers microsoft dpa sub-processor data protection ip address', body:[
    ...
    'Retention: acknowledgement, audit and quiz data is kept for 10 years as legal compliance evidence, then deleted. Former employees are retained for the same reason.',
    'Right to erasure (Art. 17): leavers are intentionally retained for audit — a recognised exemption where data is needed to meet a legal obligation. Document this decision; deletion happens when the 10-year retention lapses.',

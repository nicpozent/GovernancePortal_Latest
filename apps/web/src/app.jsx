  ...
  const saveDrawer = async () => {
    try {
      if (drawer.type === 'policy') {
        if (drawer.mode === 'edit') { const up = await api.updatePolicy(form.id, { name:form.name, docType:form.type, version:form.version, sharepointUrl:form.url, sharepointDriveId:form.driveId, sharepointItemId:form.itemId, dueDate:form.dueMode==='fixed'?(form.dueDate||null):null, dueDays:form.dueMode==='rolling'?(parseInt(form.dueDays,10)||null):null, reviewDate:form.reviewDate||null, ownerOid:form.ownerOid||null, versionNote:form.versionNote||null, groupIds:form.groupIds }); showToast(up && up.approvalReset ? 'New version saved — it must be re-approved before it’s visible again.' : 'Policy updated'); }
